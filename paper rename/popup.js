...
function render(candidates, selectedIdx, statuses) {
  ...
  items.forEach((meta, i) => {
    ...
    authorDiv.textContent = `${authorText} (${meta.year || '不明'})`;
    card.appendChild(authorDiv);

    // Title (2-line clamp)
    const titleDiv = document.createElement('div');
    titleDiv.className = 'card-title';
    titleDiv.textContent = meta.title || '(タイトルなし)';
    ...
    btn.textContent = i === selectedIdx ? '選択中' : '使用する';
    if (i !== selectedIdx) {
    ...
    sendBtn.textContent = isSendable ? '送信' : (statusInfo.sendStatus === 'sending' ? '送信中...' : '送信済み');
    if (isSendable) {
    ...
    badge.textContent = statusLabels[statusInfo.sendStatus] || '不明';
...
function renderEmpty() {
  const container = document.getElementById('container');
  container.innerHTML = '<div class="empty-msg">学術論文ページを開くと<br>候補が表示されます</div>';
}
